...
// La classe JwtAuthTokenFilter va filtrer les accès 
// selon les valeurs contenues dans le jeton envoyé.
public class JwtAuthTokenFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		try {
			...
			if (jwt != null && tokenProvider.validateJwtToken(jwt)) {
				// – à partir du nom d'utilisateur, on cree un UserDetails 
				// pour créer un objet d'authentification
				String username = tokenProvider.getUserNameFromJwtToken(jwt);
				
				// – à partir du nom d'utilisateur, on cree un UserDetails pour créer un objet d'authentification
	...
	// Bearer dsgdwhfhfgxhjxfhh
	private String getJwt(HttpServletRequest request) {
		String authHeader = request.getHeader("Authorization");
		if (authHeader != null && authHeader.startsWith("Bearer")) {
			return authHeader.replace("Bearer", "");
		}
		return null; // dsgdwhfhfgxhjxfhh
	}
